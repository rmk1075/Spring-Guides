package com.example.managing_transaction;
...
public class AppRunner implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        Assert.isTrue(bookingService.findAllBookings().size() == 3, "First booking shoule work with no problem");
        logger.info("Alice, Bob and Carol have been booked");

        try {
            bookingService.book("Chris", "Samuel");
        } catch (RuntimeException e) {
            logger.info("v--- The following exception is expect because 'Sameul' is too " + "big for the DB ---v");
